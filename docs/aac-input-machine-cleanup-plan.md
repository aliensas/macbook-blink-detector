...
- `--.`：进入输入管理。
- `.` / `-` / `--`：忽略。
- 菜单中的单次短眨不锁定选项；第二次短眨锁定当前项，随后等待是否出现第三次短眨。
...
5. 已完成：二级菜单 / 输入管理从“第一下短眨锁定”改为“第二下短眨锁定当时高亮项”。
6. 再下一步：把浏览器定时器层逐步改成“事件直接送入新内核”，减少旧 `blinkCodeBuffer` 责任。
7. 最后：迁移抬眉、张嘴、微笑、摇头事件解释。
